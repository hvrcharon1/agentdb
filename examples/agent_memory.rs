...
fn main() -> agentdb::Result<()> {
    ...
    // Setup relational table for raw events
    ...
    // --- 1. Store conversation events relationally ---
    println!("1. Storing conversation events...");
    db.execute_params(
        "INSERT OR IGNORE INTO events VALUES (?1, ?2, ?3, ?4)",
        &[&"evt_1", &"user_msg", &r#"{"text":"Tell me about Rust"}"#, &1700000001_i64],
    )?;
    db.execute_params(
        "INSERT OR IGNORE INTO events VALUES (?1, ?2, ?3, ?4)",
        &[&"evt_2", &"agent_reply", &r#"{"text":"Rust is a systems language focused on safety"}"#, &1700000002_i64],
    )?;

    let rows = db.query_json("SELECT * FROM events")?;
    println!("   Stored {} events", rows.len());

    // --- 2. Store thought embeddings in vector layer ---
    println!("\n2. Indexing thought embeddings...");
    let col = db.vectors().collection("thoughts", 4)?;

    let thoughts = vec![
        ("thought_rust", vec![0.9_f32, 0.1, 0.05, 0.0], "Rust is fast and memory safe"),
        ("thought_db",   vec![0.1_f32, 0.9, 0.05, 0.0], "Databases store and retrieve data"),
        ("thought_ai",   vec![0.05_f32, 0.1, 0.9, 0.0], "AI agents need persistent memory"),
        ("thought_embed", vec![0.0_f32, 0.05, 0.8, 0.9], "Embeddings capture semantic meaning"),
    ];
    ...
    // Search for thoughts similar to "Rust performance"
    let query = vec![0.85_f32, 0.1, 0.1, 0.0];
    let results = col.search(&query, Default::default())?;
    println!("   Top match for 'Rust performance': {:?}", 
    ...
    // Nodes: sessions, concepts, entities
    ...
    graph.add_node("concept_perf", "concept", Some(json!({"label": "Performance optimization"})))?;
    ...
    // Edges: what was discussed, with confidence weights
