...
mod tests {
    ...
    fn test_exact_match_pass() {
        assert!(filter_matches(&json!({"role":"user"}), &json!({"role":"user"})));
    }

    #[test]
    fn test_exact_match_fail() {
        assert!(!filter_matches(&json!({"role":"user"}), &json!({"role":"agent"})));
    }

    #[test]
    fn test_gt_pass() {
        assert!(filter_matches(&json!({"score":8}), &json!({"score":{"$gt":5}})));
    }

    #[test]
    fn test_gt_fail() {
        assert!(!filter_matches(&json!({"score":3}), &json!({"score":{"$gt":5}})));
    }
    ...
    #[test]
    fn test_ne() {
        assert!(filter_matches(&json!({"role":"user"}), &json!({"role":{"$ne":"agent"}})));
        assert!(!filter_matches(&json!({"role":"user"}), &json!({"role":{"$ne":"user"}})));
    }
...
mod batch_tests {
    ...
    fn open() -> AgentDB { AgentDB::open(":memory:").unwrap() }
    fn v(seed: f32) -> Vec<f32> { vec![seed, seed * 0.5, seed * 0.25, seed * 0.1] }

    #[test]
    fn test_batch_upsert_count() {
        let db = open();
        let col = db.vectors().collection("col", 4).unwrap();
        let batch = (0..50u32).map(|i| BatchEntry {
            id: format!("v{}", i),
    ...
    fn test_batch_dim_mismatch_rolls_back() {
        ...
        let col = db.vectors().collection("col", 4).unwrap();
        let batch = vec![
            BatchEntry { id: "good".into(), vector: vec![1.0,0.0,0.0,0.0], metadata: None },
            BatchEntry { id: "bad".into(),  vector: vec![1.0,0.0],         metadata: None }, // wrong dim
        ];
        assert!(col.upsert_batch(batch).is_err());
        // Collection should still be empty (rollback)
    ...
    fn test_batch_then_search() {
        let db = open();
        let col = db.vectors().collection("col", 4).unwrap();
        let batch = (0..20u32).map(|i| BatchEntry {
            id: format!("v{}", i),
    ...
    fn test_filter_gt_after_batch() {
        let db = open();
        let col = db.vectors().collection("col", 4).unwrap();
        let batch = (0..10u32).map(|i| BatchEntry {
            id: format!("v{}", i),
        ...
        assert!(results.iter().all(|r| {
            r.metadata.as_ref().unwrap()["score"].as_u64().unwrap() > 5
        }));
    }
}
...
mod hybrid_tests {
    ...
    fn open() -> AgentDB { AgentDB::open(":memory:").unwrap() }
    fn v(seed: f32) -> Vec<f32> { (0..8).map(|i| ((seed + i as f32) * 0.1).sin().abs()).collect() }
    ...
    fn test_hybrid_returns_top_k() {
        ...
        let batch = vec![
            BatchEntry { id: "a".into(), vector: v(1.0), metadata: None },
            BatchEntry { id: "b".into(), vector: v(2.0), metadata: None },
            BatchEntry { id: "c".into(), vector: v(3.0), metadata: None },
        ];
        col.upsert_batch(batch).unwrap();
    ...
    fn test_hybrid_alpha_pure_vector() {
        ...
        col.upsert_batch(vec![
            BatchEntry { id: "x".into(), vector: v(1.0), metadata: None },
        ]).unwrap();
        let graph = db.memory();
        graph.add_node("root", "session", None).unwrap();
        ...
        assert!(results[0].graph_weight == 0.0); // no graph connection
    }
    ...
    fn test_hybrid_graph_boosts_rank() {
        let db = open();
        let col = db.vectors().collection("docs", 8).unwrap();
        // Two vectors at similar distance from query
        col.upsert_batch(vec![
            BatchEntry { id: "connected".into(),    vector: v(2.0), metadata: None },
            BatchEntry { id: "disconnected".into(), vector: v(2.1), metadata: None },
        ]).unwrap();
        ...
        // disconnected has no graph edge
        ...
        let results = db.hybrid_query(HybridQuery {
            ...
            graph_depth: 1,
            top_k: 2,
            alpha: 0.4, // graph matters more
            filter: None,
        }).unwrap();

        // connected should rank above disconnected
