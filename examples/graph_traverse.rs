...
;
    ...
    graph.add_node(
        "session_01",
        "session",
        Some(json!({"user": "harshal", "date": "2025-01-01", "topic": "Rust basics"})),
    )?;
    graph.add_node(
        "session_02",
        "session",
        Some(json!({"user": "harshal", "date": "2025-01-02", "topic": "Databases"})),
    )?;
    graph.add_node(
        "session_03",
        "session",
        Some(json!({"user": "harshal", "date": "2025-01-03", "topic": "AI agents"})),
    )?;
    ...
    graph.add_node(
        "concept_perf",
        "concept",
        Some(json!({"label": "Performance"})),
    )?;
    graph.add_node("concept_db", "concept", Some(json!({"label": "Databases"})))?;
    graph.add_node(
        "concept_vectors",
        "concept",
        Some(json!({"label": "Vector search"})),
    )?;
    graph.add_node(
        "concept_agents",
        "concept",
        Some(json!({"label": "AI agents"})),
    )?;
    ...
    graph.add_node(
        "entity_agentdb",
        "entity",
        Some(json!({"label": "AgentDB", "type": "software"})),
    )?;
    graph.add_node(
        "entity_openai",
        "entity",
        Some(json!({"label": "OpenAI", "type": "company"})),
    )?;
    ...
    let agentdb_neighbors = graph.neighbors(
        "entity_agentdb",
        TraversalOptions {
            relation: None,
            max_depth: 2,
